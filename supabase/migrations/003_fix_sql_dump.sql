-- Fix generate_sql_dump: dynamic column access from generic RECORD
...
BEGIN
    ...
    LOOP
        ...
        tbl_ref := quote_ident(tbl.t);

        FOR row_rec IN EXECUTE 'SELECT ' || col_names || ' FROM ' || tbl_ref
        LOOP
            first_col := true;
            col_vals := '';
            FOR col IN
                ...
                ORDER BY ordinal_position
            LOOP
                EXECUTE 'SELECT (' || tbl_ref || '.' || quote_ident(col.column_name) || ') IS NULL'
                    INTO is_null
                    USING row_rec;
                ...
                IF is_null THEN
                    col_vals := col_vals || '\N';
                ELSE
                    EXECUTE 'SELECT (' || tbl_ref || '.' || quote_ident(col.column_name) || ')::text'
                        INTO escaped
                        USING row_rec;
