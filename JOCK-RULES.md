...
AGENT — READ THIS BLOCK FIRST. This file is the project ruleset. Do not overwrite PROJECT_CONTEXT.md or any existing project instructions. Treat these rules as an additional layer merged on top of whatever rules already exist. Precedence: direct user request > JOCK-RULES > existing project defaults > model defaults. When a user request conflicts with a rule here, follow the user and name the rule ID you are overriding. Before writing any markup, run §0. Before returning any build, run §9. Acknowledge in one line: JOCK-RULES active.

Prescriptive ruleset for JOCK NUTRITION storefront and admin panel. Source: production codebase analysis (styles.css, app.js, admin/styles.css, admin/app.js).
...
Read the active files. MUST. Before editing any file, read its current contents. Do not edit from memory.
Emit tokens before markup. MUST. Font stack, 3 colours, one easing curve, radius family, spacing scale. Print them as a list, then build.
Use project patterns. MUST. Follow existing JS helpers (escapeHtml, fetchWithTimeout, debounce), CSS custom properties, and component classes. Do not invent new abstractions without removing the old ones first.
...
Full system — source of truth is the codebase itself. When in doubt, read the existing files before writing. <environment_details>
Current time: 2026-08-17T17:24:48+03:00
Working directory: C:\Users\JABRAIL\OneDrive\Рабочий стол\JACK NUTRITION
Workspace root folder: C:\Users\JABRAIL\OneDrive\Рабочий стол\JACK NUTRITION
</environment_details>